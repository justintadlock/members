...
//add_action( 'bulk_edit_custom_box', 'quick_edit_content_permissions_box', 10, 2 );
...
function save_members_quick_edit( $post_id, $post ) {
	...
	/* Verify the nonce. */
	if ( !isset( $_REQUEST['content_permissions_meta_nonce'] ) || !wp_verify_nonce( $_REQUEST['content_permissions_meta_nonce'], plugin_basename( __FILE__ ) ) ) {
		return false;
	}
